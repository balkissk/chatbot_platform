...
from models.user_schema import UserCreate, UserLogin, UserStatusUpdate
...
class AdminUsersTest(unittest.TestCase):
    ...
    def test_create_duplicate_email_and_audit_log(self):
        created = create_user(
            UserCreate(name="New Manager", email="new@example.com", password="password123", role="manager"),
            current_user=self.admin,
            db=self.db,
        )
        ...
        with self.assertRaises(HTTPException) as raised:
            create_user(
                UserCreate(name="Duplicate", email="new@example.com", password="password123", role="manager"),
                current_user=self.admin,
                db=self.db,
            )
    ...
    def test_login_updates_last_login(self):
        created = create_user(
            UserCreate(name="Login User", email="login@example.com", password="password123", role="manager"),
            current_user=self.admin,
            db=self.db,
        )
